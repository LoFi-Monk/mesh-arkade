...
- **`resources/pear-annealing.md`** ← **READ FIRST, WRITE LAST** — living knowledge base of real project experience. More reliable than docs for anything previously encountered.
- **`resources/desktop-apps.md`** — pear-electron, pear-bridge, pear.gui config, frameless windows, window controls, known Linux issues
- **`resources/package-json.md`** — node_modules behavior, ignore patterns, stage.includes, stage.defer, assets, routes, full annotated example, common mistakes
- **`resources/staging-and-release.md`** — full staging workflow, key tracking, channel system, rollback, sharing, common confusions
- **`resources/patterns.md`** — complete copy-paste scaffolds: minimal desktop, frameless titlebar, P2P desktop, terminal app, two-peer chat, hot updates
- **`resources/p2p-stack.md`** — Hypercore, Hyperbee, Hyperdrive, Hyperswarm, HyperDHT, Corestore, Autobase full API reference
- **`resources/modules.md`** — pear-electron, pear-updates, pear-run, pear-messages, pear-crasher and other pear-* modules; deprecated API replacements
- **`resources/bare-runtime.md`** — Node.js → Bare module equivalents, Bare globals, key differences from Node