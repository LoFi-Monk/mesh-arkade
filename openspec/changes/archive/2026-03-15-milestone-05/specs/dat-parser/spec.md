...
### Requirement: SHA1 lookup
`dat-parser.ts` SHALL export a `resolveByShortSha1(records: GameRecord[], sha1: string): GameRecord | null` function. It MUST perform a case-insensitive comparison against the `sha1` field of each record and return the first match, or `null` if no match is found. This function MUST remain a pure function with no side effects.

#### Scenario: SHA1 match found
- **WHEN** `resolveByShortSha1(records, "AABB...")` is called and a record with matching SHA1 (case-insensitive) exists
- **THEN** it returns that `GameRecord`

#### Scenario: SHA1 not found
- **WHEN** `resolveByShortSha1(records, "0000...")` is called and no matching record exists
- **THEN** it returns `null`

#### Scenario: Case-insensitive comparison
- **WHEN** `resolveByShortSha1(records, "aabbcc")` is called and a record has `sha1: "AABBCC"`
- **THEN** it returns the matching record
