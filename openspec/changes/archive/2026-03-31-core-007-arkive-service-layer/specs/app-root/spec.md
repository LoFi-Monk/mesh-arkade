...
The system SHALL create the `~/mesh-arkade/` directory on first run if it does not exist. The directory SHALL contain a `config.json` file and a `DATs/` subdirectory.

#### Scenario: First run creates App Root
- **WHEN** `initAppRoot()` is called and `~/mesh-arkade/` does not exist
- **THEN** the directory is created with `config.json` and `DATs/` subdirectory
...
The `DATs/` subdirectory SHALL store raw DAT XML files after fetch. These files are user-inspectable copies alongside the Hyperbee index (per ADR-0016).

#### Scenario: DATs directory created
- **WHEN** `initAppRoot()` is called on first run
- **THEN** `~/mesh-arkade/DATs/` exists as an empty directory

#### Scenario: Raw DAT saved to cache
- **WHEN** `refreshCatalog()` fetches a DAT file
- **THEN** the raw DAT content is saved to `~/mesh-arkade/DATs/<system-name>.dat`
